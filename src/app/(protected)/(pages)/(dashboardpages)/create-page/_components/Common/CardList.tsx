...
const CardList = ({outlines,addMultipleOutlines,editText,editingCard,onCardDoubleClick,onCardSelect,onEditChange,selectedCard,setEditText,setEditingCard,setSelectedCard,addOutline}:Props) => {
    ...
    const onDrop=(e:React.DragEvent)=>{
        ...
        if(draggedIndex===-1 || dragOverIndex===draggedIndex) return;
    ...
    const onDragStart=(e:React.DragEvent,card:OutlineCard)=>{
        ...
        const onAddCard=(index?:number)=>{
            ...
            const updatedCards=index !==undefined ? [...outlines.slice(0,index+1),newCard,...outlines.slice(index+1).map((card)=>({...card,order:card.order+1})),] : [...outlines,newCard]
    ...
    <AnimatePresence>
        {
            ...
            (
                ...
                >
                    <Card onDragOver={(e)=>onDragOver(e,index)} card={card} isEditing={editingCard ===card.id} isSelected={selectedCard===card.id} editText={editText} onEditChange={onEditChange} onEditBlur={()=>onCardUpdate(card.id,editText)} onEditKeyDown={(e)=>{
                        if(e.key==='Enter'){
